gv <- c( 'FDR', 'CHR', 'BP', 'gene', 'geneId', 'pvalue', 'SNP', 'newensg',
 'chromosome', 'position', 'tissue', 'variant', 'rsid',
 'molecular_trait_id', 'maf', 'se', 'ref', 'alt')
    ...
    z <- lapply(names(resl), function(x) {
      output[[x]] <-
        DT::renderDataTable({
          if (input$focus == "chr") {
            dat <- resl[[x]]@tbl |>
              dplyr::filter(seqnames == as.character(local(input$chr))) |>
              dplyr::arrange(score) |>
              head(input$nrecs) |>
              as.data.frame()
          } else if (input$focus == "gene") {
            dat <- resl[[x]]@tbl |>
              dplyr::filter(molecular_trait_id == as.character(local(input$gene))) |>
              as.data.frame()
          } else if (input$focus == "rsid") {
            dat <- resl[[x]]@tbl |>
              dplyr::filter(rsid == as.character(local(input$snp))) |>
              as.data.frame()
          }
        })
    })
    ...
    output$all <- renderUI({
      o <- lapply(c(input$respicks, "viz"), function(x) {
        if (x != "viz") {
          ...
          tabPanel("viz", plotly::plotlyOutput("theplot"))
        }
      })
