...
data("geneNames", package="GGIpack")
...
ap = ABRIGparquet_paths()
resl = lapply(names(ap), function(x) ABRIGresource(con, tissue=x, space="hg19", pfiles=ap[x]))
names(resl) = names(ABRIGparquet_paths())
...
ui = fluidPage(
 sidebarLayout(
  sidebarPanel(
   helpText("using ABRIG eqtl data"),
